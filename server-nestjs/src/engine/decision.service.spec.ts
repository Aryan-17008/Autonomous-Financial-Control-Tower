...
describe("DecisionService", () => {
  ...
  describe("process", () => {
    ...
    it("should create BLOCK recommendation for CRITICAL alert", () => {
      const result = service.process([createAlert(Severity.CRITICAL)]);
    ...
    it("should create REVIEW recommendation for HIGH alert", () => {
      const result = service.process([createAlert(Severity.HIGH)]);
    ...
    it("should not create recommendation for MEDIUM alert", () => {
      const result = service.process([createAlert(Severity.MEDIUM)]);

      expect(result).toHaveLength(0);
    });

    it("should not create recommendation for LOW alert", () => {
      const result = service.process([createAlert(Severity.LOW)]);
  ...
  describe("calculateRiskScore", () => {
    ...
    it("should calculate risk score for a single CRITICAL alert", () => {
      expect(service.calculateRiskScore([createAlert(Severity.CRITICAL)])).toBe(
        1,
      );
    });

    it("should calculate risk score for a single HIGH alert", () => {
      expect(service.calculateRiskScore([createAlert(Severity.HIGH)])).toBe(
        0.7,
      );
    });
  ...
  describe("processSmart", () => {
    ...
    it("should use the AI enhancer when it is enabled", async () => {
      const alerts = [createAlert(Severity.HIGH)];

      const enhancedRecommendations = [
        {
          ...service.process(alerts)[0],
          reason: "AI-enhanced recommendation",
        },
      ];

      const enhance = jest.fn().mockResolvedValue(enhancedRecommendations);

      (service as any).aiEnhancer = {
        enabled: true,
        enhance,
      };
      ...
      expect(enhance).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([
          expect.objectContaining({
            ...
            action: "Review transaction txn_test_001",
            reason: "Test HIGH alert",
          }),
        ]),
      );
      expect(result).toEqual(enhancedRecommendations);
    });
